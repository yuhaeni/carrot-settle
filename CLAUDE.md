...
- **수수료 계산**: Builder 패턴 — `FeeCalculationService.builder(amount).withPgFee().withPlatformFee(grade).build()`. 각 수수료는 전용 Calculator(`PgFeeCalculator`, `PlatformFeeCalculator`)로 분리되어 `domain/fee`에 위치, api 모듈의 `FeeCalculationConfig`에서 Bean 등록. 표준 정책은 `FeeCalculationService.calculate(amount, grade)` preset으로 노출
